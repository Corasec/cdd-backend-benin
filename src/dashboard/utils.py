        ...
        for administrative_level in facilitator_administrative_levels[0][
            ...
            if len(fc_phase) < 1:
                ...
                nsc.update_cloudant_document(
                    facilitator_database, _fc_phase["_id"], _fc_phase
                )  # Update phase for the facilitator
            ...
            if len(fc_activity) < 1:
                ...
                )  # Update activity for the facilitator
...
def over_documents(develop_mode=False, training_mode=False):
    ...
    for task in tasks:
        ...
        create_task_all_facilitators(
            "process_design", task, develop_mode, training_mode
        )


def over_documents_to_add_completed_date_and_last_updated_attrs(
...
def sync_tasks_tasks_by_putting_unfinished_those_which_do_not_have_the_attachments(
    develop_mode=False, training_mode=False, no_sql_db=False
):
    if no_sql_db:
    ...
    for facilitator in facilitators:
        ...
        for _task in fc_tasks:
            ...
            if (
                ...
                and task.get("support_attachments")
            ):
                attachments = task["attachments"]
                all_attachs_filled = True

                for att in attachments:
                    if not att.get("attachment") or (
                        att.get("attachment")
                ...
                if not all_attachs_filled:
                    ...
                    nsc.update_cloudant_document(
                        facilitator_database, task["_id"], task
                    )  # Update task for the facilitator
                    print(task)
...
def create_training_facilitators(start=1, amount=1):
    ...
    while count <= amount:
        ...
        query_facilitator = Facilitator.objects.filter(id=facilitator.id).update(
            password=password
        )
        doc = {
            ...
            "phone": "123456",
            "administrative_levels": [
                {"name": "SANFATOUTE CENTRE", "id": "7e15f10d6da4ede08fa6a6810300c9ad"}
            ],
            "type": "facilitator",
        }
        nsc = NoSQLClient()
...
def clear_facilitator_documents_tasks_by_administrativelevels(
    no_sql_db, administrativelevels_ids=[]
):
    facilitators = Facilitator.objects.filter(no_sql_db_name=no_sql_db)
    nsc = NoSQLClient()
    for facilitator in facilitators:
        ...
        print(facilitator)
        nsc_database = nsc.get_db(facilitator.no_sql_db_name)
        facilitator_doc = nsc_database[
            nsc_database.get_query_result({"type": "facilitator"})[:][0]["_id"]
        ]
        ...
        for adl_id in administrativelevels_ids:
            phases = nsc_database.get_query_result(
                {"type": "phase", "administrative_level_id": adl_id}
            )
            for phase in phases:
                nsc.delete_document(nsc_database, phase["_id"])
            activities = nsc_database.get_query_result(
                {"type": "activity", "administrative_level_id": adl_id}
            )
            for activity in activities:
                nsc.delete_document(nsc_database, activity["_id"])
            tasks = nsc_database.get_query_result(
                {"type": "task", "administrative_level_id": adl_id}
            )
            for task in tasks:
                nsc.delete_document(nsc_database, task["_id"])
        ...
        doc = {"administrative_levels": _administrative_levels}
        nsc.update_doc(nsc_database, facilitator_doc["_id"], doc)


def sync_geographicalunits_with_cvd_on_facilittor(
    develop_mode=False, training_mode=False, no_sql_db=False
):
    if no_sql_db:
        facilitators = Facilitator.objects.filter(
    ...
    for facilitator in facilitators:
        ...
        for administrativelevel in doc_facilitator["administrative_levels"]:
            try:
                administrativelevel_obj = (
                    administrativelevels_models.AdministrativeLevel.objects.using(
                        "mis"
                    ).get(id=int(administrativelevel["id"]))
                )
                if administrativelevel_obj.geographical_unit:
                    geographical_unit_id_exists = False
                    for i in range(len(geographical_units)):
                        if (
                            geographical_units[i]
                            and geographical_units[i].get("sql_id")
                            and str(geographical_units[i].get("sql_id"))
                            == str(administrativelevel_obj.geographical_unit_id)
                        ):
                    ...
                    if not geographical_unit_id_exists:
                        geographical_units.append(
                            {
                                "sql_id": str(
                                    administrativelevel_obj.geographical_unit_id
                                ),
                    ...
                    for i in range(len(geographical_units)):
                        if (
                            ...
                            if administrativelevel_obj.cvd:
                                cvd_id_exists = False
                                for a in range(
                                    len(geographical_units[i].get("cvd_groups"))
                                ):
                                    if str(
                                        geographical_units[i]
                                        .get("cvd_groups")[a]
                                        .get("sql_id")
                                    ) == str(administrativelevel_obj.cvd_id):
                                        cvd_id_exists = True
                                if not cvd_id_exists:
                                    geographical_units[i].get("cvd_groups").append(
                                        {
                                            "sql_id": str(
                                                administrativelevel_obj.cvd_id
                                            ),
                                            "name": administrativelevel_obj.cvd.get_name(),
                                            "villages": [
                                                str(administrativelevel_obj.id)
                                            ],
                                        }
                                    )
                                ...
                                for a in range(
                                    ...
                                    if str(
                                        ...
                                        villages.append(str(administrativelevel_obj.id))
                                        geographical_units[i].get("cvd_groups")[a][
                                            "villages"
                                        ] = list(set(villages))
                ...
                print()
                print(
                    administrativelevel["id"],
                    administrativelevel["name"],
                    ": ",
                    exc.__str__(),
                )
                print()
        ...
        nsc.update_cloudant_document(
            facilitator_database, doc_facilitator["_id"], doc_facilitator
        )
