...
class FilterTaskForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        query_result_activities = sorted(
            query_result_activities,
            key=lambda obj: (str(obj["phase_order"]) + str(obj["order"])),
        )
        ...
        self.fields["phase"].widget.choices = get_choices(
            query_result_phases, "name", "name"
        )
        self.fields["activity"].widget.choices = get_choices(
            query_result_activities, "name", "name"
        )
        self.fields["task"].widget.choices = get_choices(
            query_result_tasks, "name", "name"
        )
...
class FacilitatorForm(forms.Form):
    error_messages = {
        ...
        "administrative_level_required": _(
